...
function! s:Scan(immediate, roots, args) "{{{3
    ...
    for root in roots
        ...
        let pos0 = len(root) + 1
        " TLogVAR pos0
        let out = ['" This file was generated by TPluginScan.']
        ...
        if index(what, 'a') != -1
            let filelist = filter(copy(files0), 'strpart(v:val, pos0) =~ ''^\f\+[\/]\(plugin\|autoload\)[\/].\{-}\.vim$''')
        else
            let filelist = filter(copy(files0), 'strpart(v:val, pos0) =~ ''^\f\+[\/]plugin[\/][^\/]\{-}\.vim$''')
        endif
        ...
        if index(what, 't') != -1
            for ftdetect in filter(copy(files0), 'strpart(v:val, pos0) =~ ''^\f\+[\/]ftdetect[\/][^\/]\{-}\.vim$''')
                call add(out, 'augroup filetypedetect')
                call extend(out, readfile(ftdetect))
                call add(out, 'augroup END')
            endfor
            let ftypes= filter(copy(files0), 'v:val =~ ''^\f\+[\/]\(ftplugin\|ftdetect\|indent\|syntax\)[\/].\{-}\.vim$''')
            " TLogVAR ftypes
            let ftd = {}
            ...
            for ftplugin in filter(copy(files0), 'strpart(v:val, pos0) =~ ''^\f\+[\/]ftplugin[\/].\{-}\.vim$''')
